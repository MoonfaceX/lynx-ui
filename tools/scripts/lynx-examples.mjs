...
const openSourceExamplesDir = join(
  currentDir,
  process.env.OPEN_SOURCE_EXAMPLES_DIR
  || '../apps/examples',
)
...
const exampleGitBaseUrl = process.env.EXAMPLE_GIT_BASE_URL
  || 'https://github.com/lynx-family/lynx-examples/tree/main'

const isPackCopy = true
const linkPath = join(currentDir, 'docs/public', 'lynx-examples')
...
function lnExampleFiles(exampleDir, lnExampleDir) {
  if (!existsSync(lnExampleDir)) {
    mkdirSync(lnExampleDir, { recursive: true })
  }
  ...
  files.forEach((file) => {
    ...
    const targetPath = join(lnExampleDir, file)
    if (statSync(fullPath).isDirectory()) {
      const dirName = basename(fullPath)
      if (ignoreDirs.includes(dirName)) {
        return
      ...
      if (isPackCopy) {
        ...
        symlinkSync(fullPath, targetPath)
      }
    } else {
      if (ignoreFiles.includes(file)) {
        return
...
function parseExampleData() {
  ...
  const processExamples = (sourceDir, isPrimary = true) => {
    if (!existsSync(sourceDir)) {
      console.warn(`Source directory not found: ${sourceDir}`)
      return
    }

    const examples = readdirSync(sourceDir)

    examples.forEach((example) => {
      const lnExampleDir = join(linkPath, example)

      // If it's a secondary source (open source) and the example already exists (from primary), skip it
      if (!isPrimary && existsSync(lnExampleDir)) {
        console.log(
          `Skipping ${example} from open source as it already exists in primary source.`,
        )
        return
      }
      ...
      const stats = statSync(exampleDir)
      ...
      const files = allFiles.map((file) => relative(exampleDir, file))
      ...
      const jsonFilePath = join(lnExampleDir, 'example-metadata.json')
  ...
  // Process primary examples first
  // processExamples(examplesDir, true)

  // Process open source examples
  processExamples(openSourceExamplesDir, false)
